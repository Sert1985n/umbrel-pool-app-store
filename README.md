...
## Как Umbrel видит обновления (и как увидеть свою панель)

**Чтобы увидеть приложение «Pool Dashboard (моя панель)», не удаляя ссылку на магазин:** зайди в App Store → открой магазин **Umbrel Pool** → нажми **Update** / **Refresh** у этого магазина → в разделе SOLO Crypto Mining появится твоя панель.

1. **В Umbrel:** зайди в магазин **Umbrel Pool** → нажми **Update** (Обновить) у этого магазина. Umbrel заново скачивает репозиторий с GitHub и подтягивает изменения.
2. **В репозитории:** в корне в `umbrel-app-store.yml` указана `version` магазина; при правках её поднимают и пушат. У каждого приложения в `umbrel-app.yml` есть своя `version` — при изменении приложения версию поднимают и пушат, тогда после Update в Umbrel будет видно обновление.
3. **Интервал:** Umbrel может кэшировать магазин; если список или приложения не обновились — снова нажми Update у магазина или перезайди в раздел App Store.
4. **Важно:** Umbrel часто не показывает бейдж «доступно обновление» для приложений из community store. Чтобы увидеть изменения — зайди в магазин **Umbrel Pool** и нажми **Update** вручную; после этого подтянутся новые версии (в т.ч. Pool Dashboard 1.0.2, магазин 1.0.3).
